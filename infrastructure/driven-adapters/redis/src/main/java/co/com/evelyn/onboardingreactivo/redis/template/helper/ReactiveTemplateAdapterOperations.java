package co.com.evelyn.onboardingreactivo.redis.template.helper;
...
public abstract class ReactiveTemplateAdapterOperations<E, K, V> {
    ...
    protected ReactiveTemplateAdapterOperations(ReactiveRedisConnectionFactory connectionFactory, ObjectMapper mapper, Function<V, E> toEntityFn) {
        ...
        Jackson2JsonRedisSerializer<V> valueSerializer = new Jackson2JsonRedisSerializer<>(dataClass);

        RedisSerializationContext<K, V> serializationContext = RedisSerializationContext
                .<K, V>newSerializationContext(keySerializer)
